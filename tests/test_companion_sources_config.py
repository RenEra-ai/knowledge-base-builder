...
# Files curated with a POSITIVE `sections` allowlist rather than a drop list.
# The allowlist must stay present and cover the plan's approved topics, so a
# revert to `sections: null` (full-file ingestion, which re-admits doctrine and
# sub-threshold XML the gate cannot catch) fails here.
_REQUIRED_SECTIONS = {
    "references/steps/data_process_groovy_step.md": {
        "core groovy patterns", "working with properties", "critical rules and gotchas",
    },
...
def test_positive_section_allowlists_stay_scoped():
    ...
    for path, required in _REQUIRED_SECTIONS.items():
        ...
        assert isinstance(sections, list) and sections, (
            f"{path}: expected a positive `sections` allowlist, got {sections!r} "
            "(a null/empty list means full-file ingestion)"
        )
